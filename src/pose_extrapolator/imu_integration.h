...
IntegrateImuResult<T> IntegrateImu(
    ...
    const double start_time, const double end_time, IteratorType* const it)
{
  ...
  {
    double next_imu_data = 9e11;
    if (std::next(*it) != imu_data.end())
    {
      next_imu_data = std::next(*it)->time;
    }
    double next_time = std::min(next_imu_data, end_time);
    const T delta_t((next_time - current_time));

    const Eigen::Matrix<T, 3, 1> delta_angle =
        (angular_velocity_calibration *
         (*it)->angular_velocity.template cast<T>()) *
        delta_t;
    result.delta_rotation *= AngleAxisVectorToRotationQuaternion(delta_angle);
...
IntegrateImu(const RangeType& imu_data, const double start_time,
             const double end_time, IteratorType* const it)
{
...
template <typename T> struct ExtrapolatePoseResult
{
  Rigid3<T> pose;
...
template <typename T, typename RangeType, typename IteratorType>
ExtrapolatePoseResult<T>
ExtrapolatePoseWithImu(const Rigid3<T>& prev_from_tracking,
                       const Eigen::Matrix<T, 3, 1>& prev_velocity_in_tracking,
                       const double prev_time,
                       const Eigen::Matrix<T, 3, 1>& gravity, const double time,
                       const RangeType& imu_data, IteratorType* const imu_it)
{
  ...
  const T delta_t = static_cast<T>((time - prev_time));
  ...
  const Eigen::Matrix<T, 3, 1> velocity =
      ...
      delta_t * gravity;
  return ExtrapolatePoseResult<T>{Rigid3<T>(translation, rotation), velocity};
}

// Same as above but given the last two poses.
template <typename T, typename RangeType, typename IteratorType>
ExtrapolatePoseResult<T> ExtrapolatePoseWithImu(
    const Rigid3<T>& prev_from_tracking, const double prev_time,
    const Rigid3<T>& prev_prev_from_tracking, const double prev_prev_time,
    const Eigen::Matrix<T, 3, 1>& gravity, const double time,
    const RangeType& imu_data, IteratorType* const imu_it)
{
  ...
  // in the optimization.
  const T prev_delta_t = static_cast<T>((prev_time - prev_prev_time));
  const Eigen::Matrix<T, 3, 1> prev_velocity_in_tracking =
